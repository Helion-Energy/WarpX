...
"""Validate one large-CFL electron-MHD whistler step under the recast.

Same broadband circularly polarized whistler as
analysis_mhd_stiff_whistler.py, but on the conservative-form recast
(implicit_mhd.fluid_flux = central): B is the JFNK field block, the Hall
EMF (J x B)/rho_q is assembled on the Ohm edges, and the block-banded LU
of the preconditioner's B block carries the whistler rows (validated
against the matrix-free operator to roundoff in-run through
pc_mhd_block.resistive_validate_assembly).

With frozen ions and u = 0 the ideal induction face fluxes vanish
identically and dB_z/dt = 0 in 1D, so the transverse recast system is
EXACTLY the linear Crank-Nicolson whistler advance per staggered Fourier
mode -- the same closed-form amplification the E-state test asserts.
"""
...
#     d B+ / dt = i*omega_h B+.
#
# The staggered Yee curl pair has k_eff=2*sin(k*dz/2)/dz and
# omega_h=D_H*k_eff^2. Crank-Nicolson therefore advances each circular
# eigenmode by the unit-modulus amplification
#
#     G_k=(1+i*a_k)/(1-i*a_k),  a_k=omega_h,k*dt/2.
#
# The recast assembles the identical staggered operator: the edge Hall
# EMF Ex/Ey = D_H dB/dz on z-nodes, curled back to the transverse faces
# (Bz stays exactly B0 -- its 1D curl row is empty -- so D_H is exactly
# constant and the transverse system exactly linear despite the
# quadratic J x B form).
...
# The 1D transverse curl has no Bz row: Bz stays exactly B0.
np.testing.assert_allclose(final_bz, background_field, rtol=1.0e-14, atol=0.0)

# Frozen ions leave rho and Ue exactly unchanged (the recast fluid RHS is
# gated off), and both must sit at the uniform references.
...
# cannot mask the count): the unpreconditioned recast solve of this
# broadband whistler state takes 65 total GMRES iterations. Require the
# banded Hall block to remove at least 90% of them (ratio 0.10, i.e. at
# most 6 iterations); the measured preconditioned count is 1 -- the
# frozen whistler rows ARE the transverse Jacobian of this linear state
# up to the O(dB/B0) perturbation coupling -- so the margin covers
# platform variation many times over.
unpreconditioned_gmres_iterations = 65
calibrated_ratio = 0.10
gmres_iterations = last_solve[7]
assert 0 < gmres_iterations <= calibrated_ratio * unpreconditioned_gmres_iterations
