...
"""Stiff RZ density-keyed hyper-resistive diffusion (hlld path, frozen ions).

A radial Bz profile decays through the corner E_theta = -eta_H lap J_theta
with the density-keyed eta_H (grid hyper number 100 at the halo minimum),
and the z-modulated eta_H bends the decay into Br. The structural
...
- B_theta stays exactly zero: with u = 0 (frozen ions) and no initial
  B_theta, E_r and E_z are pure hyper-resistive responses to J_r = J_z = 0.
...
- Br itself is excited (the z-modulated eta_H gradient does real work),
...
# The decay is real: the initial radial profile relaxes toward its
# r-weighted mean (hyper number 100 per step at the halo minimum, two
# steps taken; the measured remaining variation is ~0.62), and the
# z-modulated eta_H excites a nonzero Br.
...
variation_ratio = final_variation / initial_variation
print(f"remaining Bz variation fraction: {variation_ratio:.4f}")
assert 0.3 < variation_ratio < 0.9
assert np.max(np.abs(final_br)) > 1.0e-3 * perturbation
