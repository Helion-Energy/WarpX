...
Usage:
  ...
  analysis_m4.py --case {yee,nodal,nodal_quadratic} plotfile
      ...
      - nodal_quadratic: the m=4 anisotropy is suppressed by more than
        an order of magnitude while the m=0 smoothing bias remains
"""
...
def assert_case(case, plotfile):
    ...
    elif case == "nodal_quadratic":
        # quadratic B-spline: m=4 suppressed, m=0 smoothing remains O(h^2)
        row = rows[0.30]
        assert row["Er_m4"] <= 1.0e-5, row["Er_m4"]
        assert row["Bth_m4"] <= 1.0e-5, row["Bth_m4"]
        assert row["Er_m0"] <= 1.0e-2, row["Er_m0"]
    else:
