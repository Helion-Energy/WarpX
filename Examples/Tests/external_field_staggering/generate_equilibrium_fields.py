...
def write_A_file(fname):
    """Vector potential (mesh "A") at the edge staggering of each component."""
    file_lo = PROB_LO - NPAD * H
    nnode = NCELL + 1 + 2 * NPAD
    ...
    mesh.grid_global_offset = list(file_lo)
    mesh.grid_unit_SI = 1.0
    mesh.unit_dimension = UNIT_DIM["A"]

    for comp in ("x", "y", "z"):
        cname = "A" + comp
        pos = EDGE_POSITION[cname]
        coords = [file_lo[d] + (np.arange(nnode[d]) + pos[d]) * H[d] for d in range(3)]
        X, Y, Z = np.meshgrid(*coords, indexing="ij")
        data = np.ascontiguousarray(A_FIELD_FUNCS[cname](X, Y, Z), dtype=np.float64)
...
def main(outdir="."):
    ...
    write_A_file(f"{outdir}/ext_A_yee.h5")
