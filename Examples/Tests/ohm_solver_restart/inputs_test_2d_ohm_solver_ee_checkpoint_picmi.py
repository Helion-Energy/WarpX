...
# --- checkpoint is written. Under the energy equation T_e is evolved
# --- state: the checkpoint must carry it and the restart must restore it
# --- WITHOUT re-running either T_e seed (the uniform elec_temp fill in
# --- HybridPICModel::InitData or the adiabat seed in
# --- HybridPICInitializeRhoJandB). A restart that re-seeds T_e from rho
# --- diverges from the uninterrupted run in Te -- and, through Pe, in the
# --- fields -- at the first step after the checkpoint.
...
solver = picmi.HybridPICSolver(
    ...
    n0=n0,
    n_floor=0.05 * n0,
    # Resistivity large enough that Joule heating drives T_e well away from
    # the adiabat seed by the checkpoint step, so a restart that loses the
    # evolved T_e fails the comparison decisively.
    plasma_resistivity=1.0e-3,
