#!/usr/bin/env python3
#
# --- Whistler-dispersion gate for the generalized-Ohm's-law (GOL) E solve
# --- with electron inertia (Amano, JCP 275, 197 (2014), Fig-1 class).
# --- 1D uniform plasma with a Bz guide field and a single seeded
# --- transverse mode at k*d_e ~ 1, dumped every step through a line
# --- probe. Run twice: --esolve ohm is the massless-electron control
# --- (the standard explicit hybrid E solve), --esolve gol exercises the
# --- screened (rho_e - eps grad^2) E = F solve, whose mode must land on
# --- the inertia-corrected whistler branch (rollover
# --- w -> w_ce k^2 d_e^2 / (1 + k^2 d_e^2)) and exclude the massless
# --- branch. Fit and branch discrimination: gol_whistler_analysis.py.
# --- Configuration follows the theta-implicit inertia-seeded variant of
# --- the ohm_solver_em_modes test (Munoz et al. (2018) base parameters).
...
parser.add_argument(
    ...
    choices=["ohm", "gol"],
    default="gol",
    help="E-solve form: ohm = massless control, gol = electron-inertia "
    "screened solve (hybrid_pic_model.esolve)",
)
parser.add_argument(
    "--gol-sweeps",
    type=int,
    default=4,
    help="warm-started Jacobi sweeps per B substep (hybrid_pic_model.gol_sweeps)",
)
parser.add_argument(
    "--gol-form",
...
# Amano Eq-27 variable electron mass must sit at the PHYSICAL m_e at this
# dt, else the mode lands on an artificially heavy branch: check the
# stability bound at the B-substep cadence the lock-step E solve uses.
...
if args.esolve == "gol":
    ...
    pywarpx.hybridpicmodel.gol_n_min = 1e-6 * n_plasma
