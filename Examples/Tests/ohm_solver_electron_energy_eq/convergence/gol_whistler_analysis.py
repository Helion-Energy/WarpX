#!/usr/bin/env python3
#
# --- Analysis for the GOL whistler-dispersion gate (gol_whistler_gate.py).
# --- Fits the complex frequency of the seeded k*d_e ~ 1 transverse mode
# --- from the per-step line probe and discriminates between BOTH analytic
# --- branches of the parallel R-mode of the quasineutral hybrid
# --- dispersion: the electron-inertia-corrected branch (whistler
# --- rollover) and the massless-electron branch (about a factor 2
# --- higher). The gate: the ohm arm must land on the massless branch and
# --- the gol arm on the inertia branch, each within tolerance AND at
# --- > 0.9 of the branch gap from the wrong branch (an absolute
# --- tolerance alone can pass while sitting on the wrong branch).
# ---
# --- Discretization: the theory takes an effective wavenumber per
# --- operator. The curl k_eff is read from the OHM arm (self-
# --- calibration among the continuum/staggered/centered candidates);
# --- the GOL grad^2 screening stencil is (2 - 2 cos(k dz))/dz^2, i.e.
# --- the staggered k_eff squared, exactly.
# ---
# --- usage: gol_whistler_analysis.py <ohm_run_dir> <gol_run_dir>
...
class Arm:
    def __init__(self, run_dir):
        ...
        self.k_scr = self.k_cands["staggered"]  # the gol grad^2 stencil
    ...
    def dispersion_root(self, inertia, k_curl, seed):
        ...
        screening is form-dependent: the jacobi form screens through
        the grad^2 stencil (r = k_scr^2/k_curl^2), while the relax
        form carries TRUE dJe/dt dynamics with no spatial screening
        stencil (r = 1)."""
        if self.p.get("gol_form", "jacobi") == "relax":
...
ohm = Arm(sys.argv[1])
gol = Arm(sys.argv[2])
assert ohm.p["esolve"] == "ohm" and gol.p["esolve"] == "gol", (
    "pass the run dirs as <ohm> <gol>"
)

print(
    f"loaded: ohm {len(ohm.t)} dumps, gol {len(gol.t)} dumps, "
    f"k*d_e = {ohm.K * ohm.d_e:.4f}"
)

lbl_o, w_ohm, g_ohm = ohm.measure()
lbl_g, w_gol, g_gol = gol.measure()
print(
    f"measured: ohm ({lbl_o}) w = {w_ohm / ohm.w_ci:.3f} w_ci, "
    f"gol ({lbl_g}) w = {w_gol / gol.w_ci:.3f} w_ci"
)

# --- curl k_eff self-calibration on the ohm arm ----------------------------
print("ohm arm vs massless branch per curl-k candidate:")
best = None
for name, kc in ohm.k_cands.items():
    w_ml = ohm.dispersion_root(False, kc, 60 * ohm.w_ci)
    err = abs(w_ohm - w_ml.real) / w_ml.real
    print(f"  {name:10s}: w_ml = {w_ml.real / ohm.w_ci:7.2f} w_ci, rel err = {err:.3%}")
...
failures = []
for arm, w_meas, want_inertia in ((ohm, w_ohm, False), (gol, w_gol, True)):
...
if failures:
    ...
    sys.exit(1)
print(
    "PASS: ohm arm on the massless branch, gol arm on the "
