...
#   endif
    ...
    if (lev > 0)
    {
        ...
        // The hybrid-PIC solver does not evolve fields on the coarse patch:
        // E is recomputed on each level and B is interpolated into fine-patch
        // ghost cells, so no coarse-patch E/B fields are registered.
        if (WarpX::electromagnetic_solver_id != ElectromagneticSolverAlgo::HybridPIC)
        {
...
#   endif
            ...
            // The hybrid-PIC solver does not evolve fields on the coarse patch,
            // so no coarse-patch solver is needed.
