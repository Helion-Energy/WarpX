/* Copyright 2026 The WarpX Community
 *
 * This file is part of WarpX.
 *
 * Authors: S. Eric Clark (Helion Energy)
 *
 * License: BSD-3-Clause-LBNL
 */
#include "FluxProbes.H"

#include "YeeLoopKernel.H"

#include "Utils/TextMsg.H"
#include "WarpX.H"

#include <AMReX_GpuLaunch.H>
#include <AMReX_GpuReduce.H>
#include <AMReX_GpuParallelReduce.H>
#include <AMReX_iMultiFab.H>
#include <AMReX_ParallelDescriptor.H>
#include <AMReX_Reduce.H>

#include <cmath>
#include <memory>
#include <utility>
#include <vector>

using namespace amrex;

namespace
{
    /** One geometry-keyed owner-mask cache entry. The stored
     * BoxArray/DistributionMapping copies keep the layout refs alive, so
     * the pointer-identity hit below can never alias a rebuilt layout
     * that happened to reuse the address of a destroyed one. */
    struct OwnerMaskEntry
    {
        amrex::BoxArray ba;
        amrex::DistributionMapping dm;
        amrex::IndexType typ;
        amrex::IntVect period;
        std::unique_ptr<amrex::iMultiFab> mask;
    };
    std::vector<OwnerMaskEntry> owner_mask_cache;

#if defined(WARPX_DIM_3D)
    /** Trapezoid end-weight of a Yee component at index (i, j, k): 1/2 on
     * each non-periodic domain end plane of every direction in which the
     * component is node-centered, 1 elsewhere -- the 3D generalization of
     * the RZ probes' w_r rule (there: 1/2 at i = 0 and i = nr on the
     * r-nodal J_theta). Shared with ReciprocityLinkage, LoopLinkage,
     * CouplingPowerIntegral and the batched pack, so every functional
     * built on it uses one quadrature and the coupling-power double
     * entry closes exactly. */
    struct EndWeight3D
    {
        amrex::GpuArray<int, 3> nodal{};
        amrex::GpuArray<int, 3> periodic{};
        amrex::GpuArray<int, 3> lo_node{};
        amrex::GpuArray<int, 3> hi_node{};

        AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE
        double operator() (const int i, const int j, const int k) const noexcept
        {
            const int idx[3] = {i, j, k};
            double w = 1.0;
            for (int d = 0; d < 3; ++d) {
                if (nodal[d] != 0 && periodic[d] == 0 &&
                    (idx[d] == lo_node[d] || idx[d] == hi_node[d])) {
                    w *= 0.5;
                }
            }
            return w;
        }
    };

    EndWeight3D MakeEndWeight3D (const amrex::IndexType typ,
                                 const amrex::Geometry& geom)
    {
        EndWeight3D ew;
        const amrex::Box dom = geom.Domain();
        for (int d = 0; d < 3; ++d) {
            ew.nodal[d] = typ.nodeCentered(d) ? 1 : 0;
            ew.periodic[d] = geom.isPeriodic(d) ? 1 : 0;
            ew.lo_node[d] = dom.smallEnd(d);
            ew.hi_node[d] = dom.bigEnd(d) + 1;
        }
        return ew;
    }

    /** Position shift of a staggering per direction: 0 on node-centered
     * directions, 1/2 on cell-centered ones (x = plo + (i + shift) dx). */
    amrex::GpuArray<double, 3> StaggerShift3D (const amrex::IndexType typ)
    {
        amrex::GpuArray<double, 3> s{};
        for (int d = 0; d < 3; ++d) {
            s[d] = typ.nodeCentered(d) ? 0.0 : 0.5;
        }
        return s;
    }

    /** The two z node planes bracketing z_disk and the (unclamped) linear
     * weight toward the upper one -- identical rules to the RZ disk probe
     * and to the discrete inductance table (LoopInductance.H). */
    void DiskPlanes3D (const amrex::Geometry& geom, const double z_disk,
                       int& k0, double& w)
    {
        const double dz = geom.CellSize(2);
        const double plo_z = geom.ProbLo(2);
        const int nz = geom.Domain().length(2);
        k0 = static_cast<int>(std::floor((z_disk - plo_z) / dz));
        if (k0 < 0) { k0 = 0; }
        if (k0 > nz - 1) { k0 = nz - 1; }
        w = (z_disk - (plo_z + k0 * dz)) / dz;
    }
#endif
}

namespace warpx::circuit
{

const amrex::iMultiFab&
CachedOwnerMask (const amrex::MultiFab& mf, const amrex::Periodicity& period)
{
    // The cache outlives every consumer but must not outlive AMReX (the
    // masks' arena is torn down at Finalize; releasing them from a
    // static destructor would abort).
    static bool finalize_registered = false;
    if (!finalize_registered) {
        amrex::ExecOnFinalize([] () { owner_mask_cache.clear(); });
        finalize_registered = true;
    }
    const amrex::IntVect period_vect = period.intVect();
    for (const OwnerMaskEntry& entry : owner_mask_cache) {
        if (entry.typ == mf.ixType() && entry.period == period_vect &&
            entry.ba == mf.boxArray() &&
            entry.dm == mf.DistributionMap()) {
            return *entry.mask;
        }
    }
    // A regrid retires the old BoxArray/DistributionMapping refs for the
    // lifetime of the run; drop stale entries rather than accumulating
    // them (the cache holds at most a handful of live staggerings).
    constexpr std::size_t max_entries = 16;
    if (owner_mask_cache.size() >= max_entries) {
        owner_mask_cache.clear();
    }
    owner_mask_cache.push_back(OwnerMaskEntry{
        mf.boxArray(), mf.DistributionMap(), mf.ixType(), period_vect,
        mf.OwnerMask(period)});
    return *owner_mask_cache.back().mask;
}

amrex::Real
DiskFluxLinkage (const Coil& coil, const amrex::MultiFab& Bz)
{
    BL_PROFILE("warpx::circuit::DiskFluxLinkage");
#if defined(WARPX_DIM_RZ)
    auto& warpx = WarpX::GetInstance();
    const auto& geom = warpx.Geom(0);
    const double dr = geom.CellSize(0);
    const double dz = geom.CellSize(1);
    const double plo_r = geom.ProbLo(0);
    const double plo_z = geom.ProbLo(1);
    const int nz = geom.Domain().length(1);

    double r_off, z_off;
    QuarterOffset(coil.r, coil.z, dr, dz, r_off, z_off);

    // The two bracketing node planes and the (unclamped) linear weight --
    // identical rules to the discrete self-inductance.
    int j0 = static_cast<int>(std::floor((z_off - plo_z) / dz));
    if (j0 < 0) { j0 = 0; }
    if (j0 > nz - 1) { j0 = nz - 1; }
    const double w = (z_off - (plo_z + j0 * dz)) / dz;

    // Bz is nodal in z: boxes share the bracketing node planes, so count
    // each shared point once.
    const amrex::iMultiFab& owner = CachedOwnerMask(Bz, geom.periodicity());

    ReduceOps<ReduceOpSum> reduce_op;
    ReduceData<double> reduce_data(reduce_op);
    for (MFIter mfi(Bz, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        const Box tb = mfi.tilebox();
        const auto bz = Bz.const_array(mfi);
        const auto msk = owner.const_array(mfi);
        const int jj0 = j0;
        const double ww = w;
        const double roff = r_off;
        reduce_op.eval(tb, reduce_data,
            [=] AMREX_GPU_DEVICE (int i, int j, int /*k*/) -> GpuTuple<double>
            {
                if (j != jj0 && j != jj0 + 1) { return 0.0; }
                if (msk(i, j, 0) == 0) { return 0.0; }
                const double r_c = plo_r + (i + 0.5) * dr;
                if (!(r_c < roff)) { return 0.0; }
                const double weight = (j == jj0) ? (1.0 - ww) : ww;
                return weight * static_cast<double>(bz(i, j, 0, 0)) * r_c;
            });
    }
    double flux = amrex::get<0>(reduce_data.value(reduce_op));
    ParallelDescriptor::ReduceRealSum(flux);
    flux *= 2.0 * ablastr::coils::pi_ring * dr;

    return static_cast<amrex::Real>(flux * coil.I_ref * coil.n_turns);
#elif defined(WARPX_DIM_3D)
    auto& warpx = WarpX::GetInstance();
    const auto& geom = warpx.Geom(0);
    const double dx = geom.CellSize(0);
    const double dy = geom.CellSize(1);
    const double plo_x = geom.ProbLo(0);
    const double plo_y = geom.ProbLo(1);
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        !Bz.ixType().nodeCentered(0) && !Bz.ixType().nodeCentered(1) &&
            Bz.ixType().nodeCentered(2),
        "DiskFluxLinkage (3D) expects B_z on the Yee (cell, cell, node) "
        "staggering");

    // The nominal circle (the 3D unit field is painted at the nominal
    // filament position), its bracketing node planes and linear weight.
    int k0;
    double w;
    DiskPlanes3D(geom, coil.z, k0, w);
    const double r2 = static_cast<double>(coil.r) * static_cast<double>(coil.r);

    // Bz is nodal in z: boxes share the bracketing node planes (and
    // periodic images), so count each shared point once.
    const amrex::iMultiFab& owner = CachedOwnerMask(Bz, geom.periodicity());

    ReduceOps<ReduceOpSum> reduce_op;
    ReduceData<double> reduce_data(reduce_op);
    for (MFIter mfi(Bz, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        const Box tb = mfi.tilebox();
        const auto bz = Bz.const_array(mfi);
        const auto msk = owner.const_array(mfi);
        const int kk0 = k0;
        const double ww = w;
        reduce_op.eval(tb, reduce_data,
            [=] AMREX_GPU_DEVICE (int i, int j, int k) -> GpuTuple<double>
            {
                if (k != kk0 && k != kk0 + 1) { return 0.0; }
                if (msk(i, j, k) == 0) { return 0.0; }
                const double x_c = plo_x + (i + 0.5) * dx;
                const double y_c = plo_y + (j + 0.5) * dy;
                // STRICT squared cell-center radius test, the identical
                // expression as the discrete inductance table
                if (!(x_c * x_c + y_c * y_c < r2)) { return 0.0; }
                const double weight = (k == kk0) ? (1.0 - ww) : ww;
                return weight * static_cast<double>(bz(i, j, k, 0));
            });
    }
    double flux = amrex::get<0>(reduce_data.value(reduce_op));
    ParallelDescriptor::ReduceRealSum(flux);
    flux *= dx * dy;

    return static_cast<amrex::Real>(flux * coil.I_ref * coil.n_turns);
#else
    amrex::ignore_unused(coil, Bz);
    WARPX_ABORT_WITH_MESSAGE(
        "DiskFluxLinkage is implemented in RZ (m = 0) and 3D geometry");
    return 0.0_rt;
#endif
}

amrex::Real
ReciprocityLinkage (const VectorFieldPtrs& A, const VectorFieldPtrs& J)
{
    BL_PROFILE("warpx::circuit::ReciprocityLinkage");
#if defined(WARPX_DIM_RZ)
    const amrex::MultiFab& A_theta = *A[1];
    const amrex::MultiFab& J_theta = *J[1];
    auto& warpx = WarpX::GetInstance();
    const auto& geom = warpx.Geom(0);
    const double dr = geom.CellSize(0);
    const double dz = geom.CellSize(1);
    const double plo_r = geom.ProbLo(0);
    const int nr = geom.Domain().length(0);

    // Both fields are fully nodal: shared nodes (box faces, periodic
    // images) must be counted once.
    const amrex::iMultiFab& owner =
        CachedOwnerMask(J_theta, geom.periodicity());

    ReduceOps<ReduceOpSum> reduce_op;
    ReduceData<double> reduce_data(reduce_op);
    for (MFIter mfi(J_theta, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        const Box tb = mfi.tilebox(amrex::IntVect(1));
        const auto a = A_theta.const_array(mfi);
        const auto jt = J_theta.const_array(mfi);
        const auto msk = owner.const_array(mfi);
        const int nr_l = nr;
        reduce_op.eval(tb, reduce_data,
            [=] AMREX_GPU_DEVICE (int i, int j, int /*k*/) -> GpuTuple<double>
            {
                if (msk(i, j, 0) == 0) { return 0.0; }
                const double r = plo_r + i * dr;
                // trapezoid end-weights in r only
                const double w_r = (i == 0 || i == nr_l) ? 0.5 : 1.0;
                return w_r * r * static_cast<double>(a(i, j, 0, 0))
                       * static_cast<double>(jt(i, j, 0, 0));
            });
    }
    double lam = amrex::get<0>(reduce_data.value(reduce_op));
    ParallelDescriptor::ReduceRealSum(lam);
    lam *= 2.0 * ablastr::coils::pi_ring * dr * dz;

    return static_cast<amrex::Real>(lam);
#elif defined(WARPX_DIM_3D)
    auto& warpx = WarpX::GetInstance();
    const auto& geom = warpx.Geom(0);
    const double dV = geom.CellSize(0) * geom.CellSize(1) * geom.CellSize(2);

    // Component-wise on the matched E/J staggerings; each component's
    // shared points (box faces, periodic images) counted once.
    double lam = 0.0;
    for (int c = 0; c < 3; ++c) {
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(A[c] != nullptr && J[c] != nullptr,
            "ReciprocityLinkage (3D) needs all three A and J components");
        const amrex::MultiFab& amf = *A[c];
        const amrex::MultiFab& jmf = *J[c];
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(amf.ixType() == jmf.ixType(),
            "ReciprocityLinkage (3D): A and J components must share a "
            "staggering");
        const amrex::iMultiFab& owner =
            CachedOwnerMask(jmf, geom.periodicity());
        const EndWeight3D ew = MakeEndWeight3D(jmf.ixType(), geom);

        ReduceOps<ReduceOpSum> reduce_op;
        ReduceData<double> reduce_data(reduce_op);
        for (MFIter mfi(jmf, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            const Box tb = mfi.tilebox();
            const auto a = amf.const_array(mfi);
            const auto jc = jmf.const_array(mfi);
            const auto msk = owner.const_array(mfi);
            reduce_op.eval(tb, reduce_data,
                [=] AMREX_GPU_DEVICE (int i, int j, int k) -> GpuTuple<double>
                {
                    if (msk(i, j, k) == 0) { return 0.0; }
                    return ew(i, j, k) * static_cast<double>(a(i, j, k, 0))
                           * static_cast<double>(jc(i, j, k, 0));
                });
        }
        lam += amrex::get<0>(reduce_data.value(reduce_op));
    }
    ParallelDescriptor::ReduceRealSum(lam);
    lam *= dV;

    return static_cast<amrex::Real>(lam);
#else
    amrex::ignore_unused(A, J);
    WARPX_ABORT_WITH_MESSAGE(
        "ReciprocityLinkage is implemented in RZ (m = 0) and 3D geometry");
    return 0.0_rt;
#endif
}

amrex::Real
LoopLinkage (const Coil& coil, const VectorFieldPtrs& J,
             const double exclusion_radius)
{
    BL_PROFILE("warpx::circuit::LoopLinkage");
#if defined(WARPX_DIM_RZ)
    const amrex::MultiFab& J_theta = *J[1];
    auto& warpx = WarpX::GetInstance();
    const auto& geom = warpx.Geom(0);
    const double dr = geom.CellSize(0);
    const double dz = geom.CellSize(1);
    const double plo_r = geom.ProbLo(0);
    const double plo_z = geom.ProbLo(1);
    const int nr = geom.Domain().length(0);
    const int nz = geom.Domain().length(1);

    // The filament AS DECLARED (no quarter-cell offset added: the
    // reference probe receives the already placed filament), the coil's
    // reference amp-turns, and the mask radius.
    const double r_c = coil.r;
    const double z_c = coil.z;
    const double amps = coil.I_ref * coil.n_turns;
    const double excl2 = exclusion_radius * exclusion_radius;

    const amrex::iMultiFab& owner =
        CachedOwnerMask(J_theta, geom.periodicity());

    ReduceOps<ReduceOpSum> reduce_op;
    ReduceData<double> reduce_data(reduce_op);
    for (MFIter mfi(J_theta, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        const Box tb = mfi.tilebox(amrex::IntVect(1));
        const auto jt = J_theta.const_array(mfi);
        const auto msk = owner.const_array(mfi);
        const int nr_l = nr;
        const int nz_l = nz;
        reduce_op.eval(tb, reduce_data,
            [=] AMREX_GPU_DEVICE (int i, int j, int /*k*/) -> GpuTuple<double>
            {
                // shared nodes once; the upper axial node plane excluded
                if (msk(i, j, 0) == 0 || j == nz_l) { return 0.0; }
                const double r = plo_r + i * dr;
                const double z = plo_z + j * dz;
                const double d2 = (r - r_c) * (r - r_c) + (z - z_c) * (z - z_c);
                if (d2 < excl2) { return 0.0; }   // the exclusion mask
                // trapezoid end-weights in r only
                const double w_r = (i == 0 || i == nr_l) ? 0.5 : 1.0;
                return w_r * r * amps * YeeLoopATheta(r, z, r_c, z_c)
                       * static_cast<double>(jt(i, j, 0, 0));
            });
    }
    double lam = amrex::get<0>(reduce_data.value(reduce_op));
    ParallelDescriptor::ReduceRealSum(lam);
    lam *= 2.0 * ablastr::coils::pi_ring * dr * dz;

    return static_cast<amrex::Real>(lam);
#elif defined(WARPX_DIM_3D)
    auto& warpx = WarpX::GetInstance();
    const auto& geom = warpx.Geom(0);
    const auto dx = geom.CellSizeArray();
    const auto plo = geom.ProbLoArray();
    const double dV = dx[0] * dx[1] * dx[2];

    // The filament AS DECLARED (the 3D painting convention: nominal
    // position, no quarter-cell offset), the coil's reference amp-turns,
    // and the mask radius measured from the filament ring.
    const double r_c = coil.r;
    const double z_c = coil.z;
    const double amps = coil.I_ref * coil.n_turns;
    const double excl2 = exclusion_radius * exclusion_radius;

    // A_z of a loop is zero: the x and y components only.
    double lam = 0.0;
    for (int c = 0; c < 2; ++c) {
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(J[c] != nullptr,
            "LoopLinkage (3D) needs the J_x and J_y components");
        const amrex::MultiFab& jmf = *J[c];
        const amrex::iMultiFab& owner =
            CachedOwnerMask(jmf, geom.periodicity());
        const EndWeight3D ew = MakeEndWeight3D(jmf.ixType(), geom);
        const auto shift = StaggerShift3D(jmf.ixType());
        const bool is_x = (c == 0);

        ReduceOps<ReduceOpSum> reduce_op;
        ReduceData<double> reduce_data(reduce_op);
        for (MFIter mfi(jmf, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            const Box tb = mfi.tilebox();
            const auto jc = jmf.const_array(mfi);
            const auto msk = owner.const_array(mfi);
            reduce_op.eval(tb, reduce_data,
                [=] AMREX_GPU_DEVICE (int i, int j, int k) -> GpuTuple<double>
                {
                    if (msk(i, j, k) == 0) { return 0.0; }
                    const double x = plo[0] + (i + shift[0]) * dx[0];
                    const double y = plo[1] + (j + shift[1]) * dx[1];
                    const double z = plo[2] + (k + shift[2]) * dx[2];
                    const double rho = std::sqrt(x * x + y * y);
                    if (rho <= 0.0) { return 0.0; }
                    const double d2 = (rho - r_c) * (rho - r_c)
                                      + (z - z_c) * (z - z_c);
                    if (d2 < excl2) { return 0.0; }   // the exclusion mask
                    const double a_theta =
                        amps * YeeLoopATheta(rho, z, r_c, z_c);
                    const double a_c = is_x ? -a_theta * y / rho
                                            :  a_theta * x / rho;
                    return ew(i, j, k) * a_c
                           * static_cast<double>(jc(i, j, k, 0));
                });
        }
        lam += amrex::get<0>(reduce_data.value(reduce_op));
    }
    ParallelDescriptor::ReduceRealSum(lam);
    lam *= dV;

    return static_cast<amrex::Real>(lam);
#else
    amrex::ignore_unused(coil, J, exclusion_radius);
    WARPX_ABORT_WITH_MESSAGE(
        "LoopLinkage is implemented in RZ (m = 0) and 3D geometry");
    return 0.0_rt;
#endif
}

amrex::Real
CouplingPowerIntegral (const VectorFieldPtrs& J, const VectorFieldPtrs& E)
{
#if defined(WARPX_DIM_RZ)
    auto& warpx = WarpX::GetInstance();
    const auto& geom = warpx.Geom(0);
    const double dr = geom.CellSize(0);
    const double dz = geom.CellSize(1);
    const double plo_r = geom.ProbLo(0);

    const int nr = geom.Domain().length(0);
    double total = 0.0;
    for (int idir = 0; idir < 3; ++idir) {
        const amrex::MultiFab& jmf = *J[idir];
        const amrex::MultiFab& emf = *E[idir];
        const amrex::iMultiFab& owner =
            CachedOwnerMask(jmf, geom.periodicity());
        const bool r_nodal = jmf.ixType().nodeCentered(0);

        ReduceOps<ReduceOpSum> reduce_op;
        ReduceData<double> reduce_data(reduce_op);
        for (MFIter mfi(jmf, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            const Box tb = mfi.tilebox();
            const auto ja = jmf.const_array(mfi);
            const auto ea = emf.const_array(mfi);
            const auto msk = owner.const_array(mfi);
            const int nr_l = nr;
            reduce_op.eval(tb, reduce_data,
                [=] AMREX_GPU_DEVICE (int i, int j, int /*k*/) -> GpuTuple<double>
                {
                    if (msk(i, j, 0) == 0) { return 0.0; }
                    const double r = plo_r + (r_nodal ? i : (i + 0.5)) * dr;
                    // the same trapezoid end-weights in r as the
                    // reciprocity linkage, so the coupling-power double
                    // entry closes exactly
                    const double w_r =
                        (r_nodal && (i == 0 || i == nr_l)) ? 0.5 : 1.0;
                    return w_r * r * static_cast<double>(ja(i, j, 0, 0))
                               * static_cast<double>(ea(i, j, 0, 0));
                });
        }
        total += amrex::get<0>(reduce_data.value(reduce_op));
    }
    ParallelDescriptor::ReduceRealSum(total);
    total *= 2.0 * ablastr::coils::pi_ring * dr * dz;
    return static_cast<amrex::Real>(total);
#elif defined(WARPX_DIM_3D)
    auto& warpx = WarpX::GetInstance();
    const auto& geom = warpx.Geom(0);
    const double dV = geom.CellSize(0) * geom.CellSize(1) * geom.CellSize(2);

    double total = 0.0;
    for (int idir = 0; idir < 3; ++idir) {
        const amrex::MultiFab& jmf = *J[idir];
        const amrex::MultiFab& emf = *E[idir];
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(jmf.ixType() == emf.ixType(),
            "CouplingPowerIntegral (3D): J and E components must share a "
            "staggering");
        const amrex::iMultiFab& owner =
            CachedOwnerMask(jmf, geom.periodicity());
        // the same end-weight rule as the reciprocity linkage, so the
        // coupling-power double entry closes exactly
        const EndWeight3D ew = MakeEndWeight3D(jmf.ixType(), geom);

        ReduceOps<ReduceOpSum> reduce_op;
        ReduceData<double> reduce_data(reduce_op);
        for (MFIter mfi(jmf, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            const Box tb = mfi.tilebox();
            const auto ja = jmf.const_array(mfi);
            const auto ea = emf.const_array(mfi);
            const auto msk = owner.const_array(mfi);
            reduce_op.eval(tb, reduce_data,
                [=] AMREX_GPU_DEVICE (int i, int j, int k) -> GpuTuple<double>
                {
                    if (msk(i, j, k) == 0) { return 0.0; }
                    return ew(i, j, k) * static_cast<double>(ja(i, j, k, 0))
                           * static_cast<double>(ea(i, j, k, 0));
                });
        }
        total += amrex::get<0>(reduce_data.value(reduce_op));
    }
    ParallelDescriptor::ReduceRealSum(total);
    total *= dV;
    return static_cast<amrex::Real>(total);
#else
    amrex::ignore_unused(J, E);
    WARPX_ABORT_WITH_MESSAGE(
        "CouplingPowerIntegral is implemented in RZ (m = 0) and 3D geometry");
    return 0.0_rt;
#endif
}

void
LinkageBatch::BuildPack (const CoilSet& coils,
                         const std::vector<ProbeKind>& probes,
                         const std::vector<double>& exclusion_radius,
                         const std::vector<VectorFieldPtrs>& a_ext,
                         const amrex::MultiFab* bz,
                         const VectorFieldPtrs& j)
{
#if !defined(WARPX_DIM_RZ) && !defined(WARPX_DIM_3D)
    amrex::ignore_unused(coils, probes, exclusion_radius, a_ext, bz, j);
    WARPX_ABORT_WITH_MESSAGE(
        "LinkageBatch is implemented in RZ (m = 0) and 3D geometry");
#else
    auto& warpx = WarpX::GetInstance();
    const auto& geom = warpx.Geom(0);

    m_ncoils = coils.size();
    for (auto& jobs : m_jobs_j) { jobs.clear(); }
    m_jobs_bz.clear();

    constexpr int threads_per_block = 256;
    constexpr int max_blocks_per_job = 64;

    long weight_total = 0;
    long partial_total = 0;
    std::vector<std::vector<std::pair<long, int>>> row_segments(m_ncoils);

    auto append_job = [&](std::map<int, std::vector<Job>>& jobs,
                          const int box_index, const int row, const int comp,
                          const amrex::Box& box)
    {
        const long ncells = static_cast<long>(box.numPts());
        const int nblocks = static_cast<int>(std::min<long>(
            max_blocks_per_job,
            (ncells + threads_per_block - 1) / threads_per_block));
        jobs[box_index].push_back(
            Job{row, comp, box, weight_total, partial_total, nblocks});
        row_segments[row].emplace_back(partial_total, nblocks);
        weight_total += ncells;
        partial_total += nblocks;
        return jobs[box_index].back();
    };

    // The J components the J-based rows read: RZ the theta component
    // only, 3D all three (the loop rows skip A_z = 0).
#if defined(WARPX_DIM_RZ)
    const double dr = geom.CellSize(0);
    const double dz = geom.CellSize(1);
    const double plo_r = geom.ProbLo(0);
    const double plo_z = geom.ProbLo(1);
    const int nr = geom.Domain().length(0);
    const int nz = geom.Domain().length(1);
    const int comp_begin = 1;
    const int comp_end = 2;
#else
    const auto dx = geom.CellSizeArray();
    const auto plo = geom.ProbLoArray();
    const int comp_begin = 0;
    const int comp_end = 3;
#endif

    // Enumerate the jobs (host) in a fixed order: reciprocity and loop
    // rows over the J boxes (component by component), then disk rows over
    // the Bz boxes.
    for (int c = comp_begin; c < comp_end; ++c) {
        if (j[c] == nullptr) { continue; }
        for (amrex::MFIter mfi(*j[c]); mfi.isValid(); ++mfi) {
            const amrex::Box vb = mfi.validbox();
            for (int ic = 0; ic < m_ncoils; ++ic) {
                if (probes[ic] == ProbeKind::reciprocity ||
                    (probes[ic] == ProbeKind::loop && c != 2)) {
                    append_job(m_jobs_j[c], mfi.index(), ic, c, vb);
                }
            }
        }
    }
    if (bz != nullptr) {
        for (amrex::MFIter mfi(*bz); mfi.isValid(); ++mfi) {
            const amrex::Box vb = mfi.validbox();
            for (int ic = 0; ic < m_ncoils; ++ic) {
                if (probes[ic] != ProbeKind::disk) { continue; }
                const Coil& c = coils.coil(ic);
#if defined(WARPX_DIM_RZ)
                double r_off, z_off;
                QuarterOffset(c.r, c.z, dr, dz, r_off, z_off);
                int j0 = static_cast<int>(std::floor((z_off - plo_z) / dz));
                if (j0 < 0) { j0 = 0; }
                if (j0 > nz - 1) { j0 = nz - 1; }
                amrex::Box planes = vb;
                planes.setSmall(1, std::max(vb.smallEnd(1), j0));
                planes.setBig(1, std::min(vb.bigEnd(1), j0 + 1));
#else
                int k0;
                double w_unused;
                DiskPlanes3D(geom, c.z, k0, w_unused);
                amrex::Box planes = vb;
                planes.setSmall(2, std::max(vb.smallEnd(2), k0));
                planes.setBig(2, std::min(vb.bigEnd(2), k0 + 1));
#endif
                if (planes.isEmpty()) { continue; }
                append_job(m_jobs_bz, mfi.index(), ic, 2, planes);
            }
        }
    }

    m_weights.resize(weight_total);
    m_partials.resize(partial_total);
    m_lambda_device.resize(m_ncoils);
    m_exchange_device.resize(m_ncoils);
    m_lambda_host.resize(m_ncoils);

    // Fixed-order combine segments per row.
    std::vector<int> seg_begin(m_ncoils + 1, 0);
    std::vector<long> seg_partial_offset;
    std::vector<int> seg_nblocks;
    for (int ic = 0; ic < m_ncoils; ++ic) {
        seg_begin[ic + 1] = seg_begin[ic]
            + static_cast<int>(row_segments[ic].size());
        for (const auto& [offset, nblocks] : row_segments[ic]) {
            seg_partial_offset.push_back(offset);
            seg_nblocks.push_back(nblocks);
        }
    }
    m_seg_begin.resize(seg_begin.size());
    m_seg_partial_offset.resize(std::max<std::size_t>(1, seg_partial_offset.size()));
    m_seg_nblocks.resize(std::max<std::size_t>(1, seg_nblocks.size()));
    amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice, seg_begin.begin(),
                          seg_begin.end(), m_seg_begin.begin());
    amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice, seg_partial_offset.begin(),
                          seg_partial_offset.end(), m_seg_partial_offset.begin());
    amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice, seg_nblocks.begin(),
                          seg_nblocks.end(), m_seg_nblocks.begin());

    // Fill the weight tables device-side with the single-coil probes'
    // exact integrand factors (owner masks, radius/trapezoid/plane
    // weights, volume factors, the disk probe's reference amp-turns).
    double* const AMREX_RESTRICT weights = m_weights.data();
    for (int c = comp_begin; c < comp_end; ++c) {
        if (j[c] == nullptr || m_jobs_j[c].empty()) { continue; }
        const amrex::MultiFab& jmf = *j[c];
        const amrex::iMultiFab& owner =
            CachedOwnerMask(jmf, geom.periodicity());
#if defined(WARPX_DIM_3D)
        const EndWeight3D ew = MakeEndWeight3D(jmf.ixType(), geom);
        const auto shift = StaggerShift3D(jmf.ixType());
        const double factor = dx[0] * dx[1] * dx[2];
        const bool is_x = (c == 0);
#endif
        for (amrex::MFIter mfi(jmf); mfi.isValid(); ++mfi) {
            const auto it = m_jobs_j[c].find(mfi.index());
            if (it == m_jobs_j[c].end()) { continue; }
            const auto msk = owner.const_array(mfi);
            for (const Job& job : it->second) {
                const amrex::Box box = job.box;
                const long w_offset = job.weight_offset;
#if defined(WARPX_DIM_RZ)
                const int nr_l = nr;
                const double factor =
                    2.0 * ablastr::coils::pi_ring * dr * dz;
                const double plo_r_l = plo_r;
                const double dr_l = dr;
                if (probes[job.row] == ProbeKind::loop) {
                    // LoopLinkage's integrand: the analytic loop A of the
                    // declared filament, masked within the exclusion
                    // radius, the upper axial node plane excluded.
                    const Coil& cl = coils.coil(job.row);
                    const double r_c = cl.r;
                    const double z_c = cl.z;
                    const double amps = cl.I_ref * cl.n_turns;
                    const double excl2 = exclusion_radius[job.row]
                                         * exclusion_radius[job.row];
                    const double plo_z_l = plo_z;
                    const double dz_l = dz;
                    const int nz_l = nz;
                    amrex::ParallelFor(box,
                        [=] AMREX_GPU_DEVICE (int i, int jj, int /*k*/)
                        {
                            const amrex::Box b = box;
                            const long lin = b.index(amrex::IntVect(i, jj));
                            const double r = plo_r_l + i * dr_l;
                            const double z = plo_z_l + jj * dz_l;
                            const double w_r =
                                (i == 0 || i == nr_l) ? 0.5 : 1.0;
                            const double d2 = (r - r_c) * (r - r_c)
                                              + (z - z_c) * (z - z_c);
                            weights[w_offset + lin] =
                                (msk(i, jj, 0) == 0 || jj == nz_l || d2 < excl2)
                                    ? 0.0
                                    : w_r * r * factor * amps
                                          * YeeLoopATheta(r, z, r_c, z_c);
                        });
                    continue;
                }
                const auto a = a_ext[job.row][1]->const_array(mfi);
                amrex::ParallelFor(box,
                    [=] AMREX_GPU_DEVICE (int i, int jj, int /*k*/)
                    {
                        const amrex::Box b = box;
                        const long lin = b.index(amrex::IntVect(i, jj));
                        const double r = plo_r_l + i * dr_l;
                        const double w_r =
                            (i == 0 || i == nr_l) ? 0.5 : 1.0;
                        weights[w_offset + lin] =
                            (msk(i, jj, 0) == 0)
                                ? 0.0
                                : w_r * r * factor
                                      * static_cast<double>(a(i, jj, 0, 0));
                    });
#else
                if (probes[job.row] == ProbeKind::loop) {
                    // LoopLinkage's 3D integrand: the analytic loop A of
                    // the declared filament projected on this component,
                    // masked within the exclusion radius of the ring.
                    const Coil& cl = coils.coil(job.row);
                    const double r_c = cl.r;
                    const double z_c = cl.z;
                    const double amps = cl.I_ref * cl.n_turns;
                    const double excl2 = exclusion_radius[job.row]
                                         * exclusion_radius[job.row];
                    amrex::ParallelFor(box,
                        [=] AMREX_GPU_DEVICE (int i, int jj, int k)
                        {
                            const amrex::Box b = box;
                            const long lin = b.index(amrex::IntVect(i, jj, k));
                            const double x = plo[0] + (i + shift[0]) * dx[0];
                            const double y = plo[1] + (jj + shift[1]) * dx[1];
                            const double z = plo[2] + (k + shift[2]) * dx[2];
                            const double rho = std::sqrt(x * x + y * y);
                            const double d2 = (rho - r_c) * (rho - r_c)
                                              + (z - z_c) * (z - z_c);
                            if (msk(i, jj, k) == 0 || rho <= 0.0 || d2 < excl2) {
                                weights[w_offset + lin] = 0.0;
                                return;
                            }
                            const double a_theta =
                                amps * YeeLoopATheta(rho, z, r_c, z_c);
                            const double a_c = is_x ? -a_theta * y / rho
                                                    :  a_theta * x / rho;
                            weights[w_offset + lin] =
                                ew(i, jj, k) * factor * a_c;
                        });
                    continue;
                }
                WARPX_ALWAYS_ASSERT_WITH_MESSAGE(a_ext[job.row][c] != nullptr,
                    "LinkageBatch (3D): a reciprocity row needs all three "
                    "unit-field components");
                const auto a = a_ext[job.row][c]->const_array(mfi);
                amrex::ParallelFor(box,
                    [=] AMREX_GPU_DEVICE (int i, int jj, int k)
                    {
                        const amrex::Box b = box;
                        const long lin = b.index(amrex::IntVect(i, jj, k));
                        weights[w_offset + lin] =
                            (msk(i, jj, k) == 0)
                                ? 0.0
                                : ew(i, jj, k) * factor
                                      * static_cast<double>(a(i, jj, k, 0));
                    });
#endif
            }
        }
    }
    if (bz != nullptr) {
        const amrex::iMultiFab& owner =
            CachedOwnerMask(*bz, geom.periodicity());
        for (amrex::MFIter mfi(*bz); mfi.isValid(); ++mfi) {
            const auto it = m_jobs_bz.find(mfi.index());
            if (it == m_jobs_bz.end()) { continue; }
            const auto msk = owner.const_array(mfi);
            for (const Job& job : it->second) {
                const Coil& c = coils.coil(job.row);
                const amrex::Box box = job.box;
                const long w_offset = job.weight_offset;
#if defined(WARPX_DIM_RZ)
                double r_off, z_off;
                QuarterOffset(c.r, c.z, dr, dz, r_off, z_off);
                int j0 = static_cast<int>(std::floor((z_off - plo_z) / dz));
                if (j0 < 0) { j0 = 0; }
                if (j0 > nz - 1) { j0 = nz - 1; }
                const double w = (z_off - (plo_z + j0 * dz)) / dz;
                const double factor = 2.0 * ablastr::coils::pi_ring * dr
                    * c.I_ref * c.n_turns;
                const double plo_r_l = plo_r;
                const double dr_l = dr;
                const double roff_l = r_off;
                const int jj0 = j0;
                amrex::ParallelFor(box,
                    [=] AMREX_GPU_DEVICE (int i, int jj, int /*k*/)
                    {
                        const amrex::Box b = box;
                        const long lin = b.index(amrex::IntVect(i, jj));
                        const double r_c = plo_r_l + (i + 0.5) * dr_l;
                        const double weight_j = (jj == jj0) ? (1.0 - w) : w;
                        weights[w_offset + lin] =
                            (msk(i, jj, 0) == 0 || !(r_c < roff_l))
                                ? 0.0
                                : weight_j * r_c * factor;
                    });
#else
                int k0;
                double w;
                DiskPlanes3D(geom, c.z, k0, w);
                const double r2 = static_cast<double>(c.r)
                                  * static_cast<double>(c.r);
                const double factor = dx[0] * dx[1] * c.I_ref * c.n_turns;
                const int kk0 = k0;
                amrex::ParallelFor(box,
                    [=] AMREX_GPU_DEVICE (int i, int jj, int k)
                    {
                        const amrex::Box b = box;
                        const long lin = b.index(amrex::IntVect(i, jj, k));
                        const double x_c = plo[0] + (i + 0.5) * dx[0];
                        const double y_c = plo[1] + (jj + 0.5) * dx[1];
                        const double weight_k = (k == kk0) ? (1.0 - w) : w;
                        weights[w_offset + lin] =
                            (msk(i, jj, k) == 0 || !(x_c * x_c + y_c * y_c < r2))
                                ? 0.0
                                : weight_k * factor;
                    });
#endif
            }
        }
    }
    amrex::Gpu::streamSynchronize();

    for (int c = 0; c < 3; ++c) {
        m_key_ba_j[c] = (j[c] != nullptr) ? j[c]->boxArray() : amrex::BoxArray{};
        m_key_dm_j[c] = (j[c] != nullptr) ? j[c]->DistributionMap()
                                          : amrex::DistributionMapping{};
    }
    m_key_ba_bz = (bz != nullptr) ? bz->boxArray() : amrex::BoxArray{};
    m_key_dm_bz = (bz != nullptr) ? bz->DistributionMap()
                                  : amrex::DistributionMapping{};
    m_key_domain = geom.Domain();
    m_built = true;
#endif
}

} // namespace warpx::circuit
