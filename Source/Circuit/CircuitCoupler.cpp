...
CircuitCoupler::MeasureLinkages (const bool refresh_plasma_current)
{
    ...
    // remain the validated reference below). The J-based probes read the
    // plasma current and the coils' unit fields by component: RZ (m = 0)
    // uses the theta entries, 3D all three.
    const amrex::MultiFab* bz = any_disk
        ? warpx.m_fields.get(FieldType::Bfield_fp,
                             ablastr::fields::Direction{2}, 0)
        : nullptr;
    VectorFieldPtrs j_plasma{nullptr, nullptr, nullptr};
    if (any_reciprocity) {
        for (int c = 0; c < 3; ++c) {
            j_plasma[c] = warpx.m_fields.get(
                FieldType::hybrid_current_fp_plasma,
                ablastr::fields::Direction{c}, 0);
        }
    }
    m_a_ext_scratch.assign(m_coils.size(),
                           VectorFieldPtrs{nullptr, nullptr, nullptr});
    for (int ic = 0; ic < m_coils.size(); ++ic) {
        if (m_probes[ic] == ProbeKind::reciprocity) {
            for (int c = 0; c < 3; ++c) {
                m_a_ext_scratch[ic][c] = warpx.m_fields.get(
                    m_coils.coil(ic).field_name + "_Aext",
                    ablastr::fields::Direction{c}, 0);
            }
        }
    }
    m_batch.Measure(m_coils, m_probes, m_probe_exclusion, m_a_ext_scratch,
                    bz, j_plasma, m_lambda_scratch);
    for (int ic = 0; ic < m_coils.size(); ++ic) {
    ...
    if (m_params.probe_crosscheck) {
        ...
        auto const& device = m_batch.MeasureDevice(m_coils, m_probes, m_probe_exclusion,
            m_a_ext_scratch, bz, j_plasma);
        amrex::Gpu::PinnedVector<double> checked(device.size());
        amrex::Gpu::copy(amrex::Gpu::deviceToHost, device.begin(), device.end(), checked.begin());
        for (int coil = 0; coil < m_coils.size(); ++coil) {
            auto const scale = std::max(std::abs(checked[coil]), std::abs(m_lambda_scratch[coil]));
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(std::abs(checked[coil] - m_lambda_scratch[coil])
                <= m_params.crosscheck_rtol*std::max(scale, 1.e-300_rt),
                "Device and host-reference circuit probes disagree");
        }
#endif
        ...
        for (int ic = 0; ic < m_coils.size(); ++ic) {
            ...
            const amrex::Real reference =
                ...
                : (kind == ProbeKind::loop)
                    ? LoopLinkage(c, j_plasma, m_probe_exclusion[ic])
                    : ReciprocityLinkage(m_a_ext_scratch[ic], j_plasma);
...
CircuitCoupler::ConfigureDarwinMagneticResponse ()
{
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(!m_params.linkage_reference_accepted
        && m_params.residual_advance_full_step,
        "Native Darwin requires linkage_reference=first_iterate and residual_advance=full_step; "
        "the reference is measured explicitly at the accepted step entry");
    for (auto const kind : m_probes) {
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(kind == warpx::circuit::ProbeKind::disk
            || kind == warpx::circuit::ProbeKind::none,
            "Native Darwin magnetic-response coupling currently requires disk probes");
    }
    auto& simulation = WarpX::GetInstance();
    auto const& external = *simulation.get_pointer_HybridPICModel()->m_external_vector_potential;
    m_darwin_external_count = external.nFields();
    m_darwin_unit_response.assign(m_coils.size()*m_darwin_external_count, 0._rt);
    for (int field = 0; field < m_darwin_external_count; ++field) {
        auto const& unit_bz = *simulation.m_fields.get(external.FieldName(field) + "_curlAext",
            ablastr::fields::Direction{2}, 0);
        for (int coil = 0; coil < m_coils.size(); ++coil) {
            if (m_probes[coil] == warpx::circuit::ProbeKind::none) { continue; }
            m_darwin_unit_response[coil*m_darwin_external_count + field] =
                warpx::circuit::DiskFluxLinkage(m_coils.coil(coil), unit_bz);
        }
    }
}
...
CircuitCoupler::ResetDarwinTrialScales (amrex::Real t0, amrex::Real dt)
{
    ...
    for (auto const& coil : m_coils.coils()) {
        auto const accepted = external.GetScale(coil.field_name, t0);
        external.SetScale(coil.field_name, accepted, accepted, t0, t0 + dt);
    }
}

void
CircuitCoupler::MeasureDarwinLinkages (amrex::Real time)
{
    BL_PROFILE("CircuitCoupler::MeasureDarwinLinkages()");
    auto const& external = *WarpX::GetInstance().get_pointer_HybridPICModel()
        ->m_external_vector_potential;
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(m_darwin_external_count == external.nFields(),
        "Darwin magnetic-response probe matrix must be configured first");
    MeasureLinkages(false);
    for (int coil = 0; coil < m_coils.size(); ++coil) {
        if (m_probes[coil] == warpx::circuit::ProbeKind::none) { continue; }
        auto& response = m_lambda.at(m_coils.coil(coil).name);
        for (int field = 0; field < m_darwin_external_count; ++field) {
            response -= external.TimeScale(field, time)
                *m_darwin_unit_response[coil*m_darwin_external_count + field];
        }
    }
    if (m_params.probe_crosscheck) {
        // Independently subtract the fields BEFORE integration. This validates
        // the response-frame adapter, including every imposed field and the
        // reference-current normalization. Cancellation is scaled by the full
        // measured terms, not by a possibly zero response.
        auto& simulation = WarpX::GetInstance();
        auto const& total = *simulation.m_fields.get(FieldType::Bfield_fp,
            ablastr::fields::Direction{2}, 0);
        amrex::MultiFab response(total.boxArray(), total.DistributionMap(), 1, 0);
        amrex::MultiFab::Copy(response, total, 0, 0, 1, 0);
        for (int field = 0; field < m_darwin_external_count; ++field) {
            auto const& unit = *simulation.m_fields.get(external.FieldName(field) + "_curlAext",
                ablastr::fields::Direction{2}, 0);
            amrex::MultiFab::Saxpy(response, -external.TimeScale(field, time), unit, 0, 0, 1, 0);
        }
        for (int coil = 0; coil < m_coils.size(); ++coil) {
            if (m_probes[coil] == warpx::circuit::ProbeKind::none) { continue; }
            auto const direct = warpx::circuit::DiskFluxLinkage(m_coils.coil(coil), response);
            auto const cached = m_lambda.at(m_coils.coil(coil).name);
            amrex::Real scale = std::abs(m_lambda_scratch[coil]);
            for (int field = 0; field < m_darwin_external_count; ++field) {
                scale += std::abs(external.TimeScale(field, time)
                    *m_darwin_unit_response[coil*m_darwin_external_count + field]);
            }
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(std::abs(direct - cached)
                <= m_params.crosscheck_rtol*std::max(scale, 1.e-300_rt),
                "Darwin magnetic response disagrees with direct field subtraction");
        }
    }
}
