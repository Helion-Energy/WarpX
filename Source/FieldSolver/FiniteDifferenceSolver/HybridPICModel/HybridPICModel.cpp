...
void HybridPICModel::ReadParameters ()
{
                    ...
                    m_cond_vacuum_fast_front);
    {
        ...
        // band acts as a perpetual bath donor (measured on the stiff-slab
    ...
    // Redirect hardening knobs (2026-08 Te-runaway forensics; see the
    ...
    // Generalized-Ohm's-law E-solve FORM (see member doc).
    // Terminology: both paths solve the generalized Ohm's law; the
    // distinction is the FORM. "e_form" (alias "ohm") = the legacy
    // algebraic solve for E with its density division; "je_form"
    // (deprecated aliases "amano_form", "gol") = the division-free
    // dynamical electron-current form: J_e advances from the full
    // electron momentum equation (Hewett & Nielson 1978), with the
    // variable-mass and vacuum-decay regularizers of Amano 2014.
    {
        std::string esolve = "e_form";
        pp_hybrid.query("esolve", esolve);
        if (esolve == "je_form" || esolve == "amano_form" ||
            esolve == "gol") {
            m_esolve_je = true;
        } else {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
                ...
                "or 'je_form' (deprecated aliases 'amano_form', 'gol')");
        }
        // The relax advance is wired on the RZ path with the Lie split.
        ...
        // Every je_* input is also read under its deprecated gol_*
        // spelling; the je_* spelling wins when both are set.
        pp_hybrid.query("gol_sweeps", m_je_sweeps);
        pp_hybrid.query("je_sweeps", m_je_sweeps);
        utils::parser::queryWithParser(pp_hybrid, "gol_cfl_alpha", m_je_alpha);
        utils::parser::queryWithParser(pp_hybrid, "je_cfl_alpha", m_je_alpha);
        utils::parser::queryWithParser(pp_hybrid, "gol_n_min", m_je_n_min);
        utils::parser::queryWithParser(pp_hybrid, "je_n_min", m_je_n_min);
        utils::parser::queryWithParser(pp_hybrid, "gol_vacuum_gamma_frac",
                                       m_je_vac_gamma_frac);
        utils::parser::queryWithParser(pp_hybrid, "je_vacuum_gamma_frac",
                                       m_je_vac_gamma_frac);
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            !m_esolve_je ||
                (m_je_n_min > 0.0_rt && m_je_sweeps >= 1 &&
                 m_je_alpha > 0.0_rt),
            "hybrid_pic_model.esolve = je_form requires je_n_min > 0 "
            "(the one-count density level), je_sweeps >= 1 and "
            "je_cfl_alpha > 0");
        // This fork ports the relax advance only (see the m_je_advance
        // member doc): relax is the default here, and an explicit
        // jacobi selection aborts rather than running unported code.
        std::string je_advance = "relax";
        pp_hybrid.query("gol_form", je_advance);
        pp_hybrid.query("je_advance", je_advance);
        if (je_advance == "relax") { m_je_advance = 1; }
        else {
            WARPX_ALWAYS_ASSERT_WITH_MESSAGE(je_advance != "jacobi",
                "hybrid_pic_model.je_advance = jacobi is not ported on "
                "this fork; use relax");
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            m_je_advance == 0 ||
                (m_je_c_frac > 0.0_rt && m_je_c_frac <= 1.0_rt),
            "hybrid_pic_model.je_c_frac must be in (0, 1]");
        utils::parser::queryWithParser(pp_hybrid, "gol_c_max", m_je_c_max);
        utils::parser::queryWithParser(pp_hybrid, "je_c_max", m_je_c_max);
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(m_je_c_max >= 0.0_rt,
        ...
        utils::parser::queryWithParser(pp_hybrid, "gol_div_clean_frac",
                                       m_je_div_clean_frac);
        utils::parser::queryWithParser(pp_hybrid, "je_div_clean_frac",
                                       m_je_div_clean_frac);
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            m_je_div_clean_frac >= 0.0_rt && m_je_div_clean_frac <= 1.0_rt,
            "hybrid_pic_model.je_div_clean_frac must be in [0, 1]");
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            !m_esolve_je || m_je_div_clean_frac == 0.0_rt,
            "Marder cleaning is not yet ported to the RZ path; set "
            "hybrid_pic_model.je_div_clean_frac = 0");
        pp_hybrid.query("gol_var_mass", m_je_var_mass);
        pp_hybrid.query("je_var_mass", m_je_var_mass);
        utils::parser::queryWithParser(pp_hybrid, "je_visc_frac",
                                       m_je_visc_frac);
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(m_je_visc_frac >= 0.0_rt,
            "hybrid_pic_model.je_visc_frac cannot be negative");
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            m_je_visc_frac == 0.0_rt || m_je_var_mass,
            "hybrid_pic_model.je_visc_frac requires je_var_mass = 1 "
    ...
    const IntVect& Bz_nodal_flag) const
{
    ...
    // Persistent electron-current state for the Je-form relax advance
    // (esolve = je_form, je_advance = relax): J_e evolves semi-implicitly in
    ...
    if (m_esolve_je && m_je_advance == 1) {
        ...
        // and only allocated -- when the electron energy equation
        // consumes it.
        if (m_je_visc_frac > 0.0_rt && m_solve_electron_energy_equation) {
...
void HybridPICModel::InitData (const ablastr::fields::MultiFabRegister& fields)
{
    ...
    // Guard rail (2026-08 Te-runaway forensics): without a relaxation
...
void HybridPICModel::JeAddAnomalousHeating (int const lev) const
{
    ABLASTR_PROFILE("HybridPICModel::JeAddAnomalousHeating()");
    ...
    amrex::MultiFab & Te =
        *warpx.m_fields.get(FieldType::hybrid_electron_temperature_fp, lev);
    amrex::MultiFab const & rho = *warpx.m_fields.get(FieldType::rho_fp, lev);
    amrex::MultiFab & Qv = *warpx.m_fields.get("hybrid_je_qvisc_fp", lev);

    auto const gamma_minus_1 = m_gamma - 1.0_rt;
    amrex::Real const rho_1c = PhysConst::q_e * m_je_n_min;
    amrex::Real const kb_over_qe = PhysConst::kb / PhysConst::q_e;

    // dT_e[K] = (gamma - 1) Q / (n_g k_B) with the one-count-guarded
    // n_g = max(rho, rho_1c)/q_e; the accumulator is cleared after use.
#ifdef AMREX_USE_OMP
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
#endif
    for (MFIter mfi(Te, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        amrex::Array4<amrex::Real> const & Te_arr = Te.array(mfi);
        amrex::Array4<amrex::Real> const & Qv_arr = Qv.array(mfi);
        amrex::Array4<amrex::Real const> const & rho_arr = rho.const_array(mfi);
        amrex::ParallelFor(mfi.tilebox(),
            [=] AMREX_GPU_DEVICE (int i, int j, int k) {
                amrex::Real const rho_g = amrex::max(rho_arr(i,j,k), rho_1c);
                Te_arr(i,j,k) +=
                    gamma_minus_1 * Qv_arr(i,j,k) / (rho_g * kb_over_qe);
                Qv_arr(i,j,k) = 0.0_rt;
            });
    }
}
                                           ...
                                           amrex::MultiFab * const redirect_E) const
{
    ...
    // numerical vacuum-regularizer ramp was the measured Te runaway's ignition
        ...
        for (MFIter mfi(Kout, TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                                      ...
                                      int const c)
                {
                    ...
                    // ghosts -- masked on the production-class decks by the
                                              ...
                                              bool const fill_te_ghosts) const
{
    ...
    // Je-form anomalous-friction heat (accumulated per substep by the
    // relax advance in hybrid_je_qvisc_fp; see the m_je_visc_frac member
    // doc) applied to T_e, accumulator cleared. Runs beside the Joule
    // source so the shunt/limiter below sees the combined heat.
    if (m_esolve_je && m_je_advance == 1 && m_je_visc_frac > 0.0_rt) {
        JeAddAnomalousHeating(lev);
    }
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
    ...
    // Graceful Te-runaway abort (>700 keV Te runaways otherwise ended
    ...
    IntVect ng, std::optional<bool> nodal_sync )
{
    ...
    // Je-form relax integration contract (see the m_je_relax_advance
    ...
    bool const je_relax = (m_esolve_je && m_je_advance == 1);
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(!(je_relax && use_rkf45),
        "hybrid_pic_model.esolve = je_form (relax advance) requires the "
