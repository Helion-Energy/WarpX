    ...
    const IntVect& Bz_nodal_flag) const
{
    ...
    // zero init = the exact pre-history for quiescent starts), the
    // per-evaluation nodal scratch for the assembled inertial field, the
    // converged theta-stage save feeding the end-of-step rotation, and
    // the per-step frozen rho^n leg of drho/dt. One ghost cell
    // throughout: the histories are read at valid nodes only and the
    // assembled field is consumed by valid-contained Interp stencils --
    // wider ghosts would never be maintained.
    if (m_include_electron_inertia) {
        ...
        fields.alloc_init("hybrid_rho_n_frozen",
            lev, amrex::convert(ba, rho_nodal_flag), dm, 1, ng1, 0.0_rt);
    }
    ...
    const bool include_resistive_terms) const
{
    auto& warpx = WarpX::GetInstance();
    HybridPICSolveE(
        Efield, Jfield, Bfield, rhofield, eb_update_E,
        include_pressure_gradient, include_resistive_terms,
        warpx.gett_new(0), true);
}
...
HybridPICModel::HybridPICSolveE (
    ...
    ablastr::fields::MultiLevelVectorField const& Bfield,
    ablastr::fields::MultiLevelScalarField const& rhofield,
    amrex::Vector<std::array<std::unique_ptr<amrex::iMultiFab>, 3>>&
        eb_update_E,
    const bool include_pressure_gradient, const bool include_resistive_terms,
    const amrex::Real evaluation_time,
    const bool execute_python_callback) const
{
    auto& warpx = WarpX::GetInstance();
    for (int lev = 0; lev <= warpx.finestLevel(); ++lev)
    {
    ...
    const bool include_resistive_terms) const
{
    ...
    HybridPICSolveE(
        Efield, Jfield, Bfield, rhofield, eb_update_E, lev,
        patch_type, include_pressure_gradient, include_resistive_terms,
        warpx.gett_new(lev));
}
...
HybridPICModel::HybridPICSolveE (
    ...
    std::array<std::unique_ptr<amrex::iMultiFab>, 3>& eb_update_E,
    const int lev, const PatchType patch_type,
    const bool include_pressure_gradient, const bool include_resistive_terms,
    const amrex::Real evaluation_time) const
{
    auto& warpx = WarpX::GetInstance();
    ...
    warpx.get_pointer_fdtd_solver_fp(lev)->HybridPICSolveE(
        ...
        *electron_pressure_fp, eb_update_E, lev, this,
        include_pressure_gradient, include_resistive_terms, evaluation_time);
    warpx.ApplyEfieldBoundary(lev, patch_type, evaluation_time);
}
                                                  ...
                                                  bool a_from_jacobian)
{
    ...
    // Effective electron mass, resolved on first use: the lightest charged
    // ion species divided by the reduced mass ratio (physical m_e when the
    // ratio is unset). The ratio moves the effective electron skin depth
    // d_e = c/omega_pe(m_e_eff) relative to the grid. The theta-implicit
    // MHD solver pre-resolves the mass from its fluid charge-to-mass ratio
    ...
    amrex::MultiFab const & rho = *warpx.m_fields.get(FieldType::rho_fp, lev);
    // Stage density and drho/dt rate (see m_inertia_recast_mode): the
    // recast reads the theta-stage fluid charge density (component 0,
    // filled from the state each evaluation) against the solver-frozen
    // step-start image, EXACT rate (rho^theta - rho^n)/(theta dt). The
    // hybrid deposit path reads the MIDPOINT deposit (component nComp/2)
    // against the first-evaluation capture, rate 2/dt -- deposits are
    // consumed raw, never divided by theta dt (only correct at
    // theta = 1/2; theta > 1/2 would time-extrapolate deposit noise).
    int rho_stage_comp = 0;
    amrex::Real drho_rate = 1.0_rt / (a_theta * a_dt);
    if (!m_inertia_recast_mode) {
    ...
    // true step-start density there. Capture it once per step on the
    // first non-Jacobian evaluation (probe states are perturbed); the
    // recast solver fills the register itself, exactly, each step.
    amrex::MultiFab const & rho_n =
        *warpx.m_fields.get("hybrid_rho_n_frozen", lev);
    if (!m_inertia_recast_mode && !m_inertia_rho_n_captured &&
        !a_from_jacobian) {
        amrex::MultiFab & rho_n_mut =
            *warpx.m_fields.get("hybrid_rho_n_frozen", lev);
        amrex::MultiFab::Copy(rho_n_mut, rho, 0, 0, 1,
                              amrex::min(rho_n_mut.nGrowVect(),
                                         rho.nGrowVect()));
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
    ...
    // Zero-extend past physical boundaries; interior/periodic ghosts from
    // the exchange.
    ...
    // write before the end-of-step update is the converged iterate's
    // assembly.
    if (!a_from_jacobian) {
    ...
    //                                           - (Je.grad)(Je/rho) ],
    // the Je-form of -(m_e_eff/e) D u_e/Dt with u_e = -Je/rho (the
    // implicit-PIC formulation of Angus et al.). The sign and the 1/rho
    // are fixed by the cold-electron limit of the electron momentum
    // equation, E = (m_e/(e^2 n_e)) dJe/dt (the collisionless inertial
    // reactance): the POSITIVE sign caps the whistler branch at
    // k d_e ~ 1 -- flipped, it manufactures the spurious resonance branch
    // instead, and only a dispersion test at k_eff d_e ~ 1 can tell the
    // two apart. dJe/dt uses the theta-extrapolated iterate
...
#pragma omp parallel if (amrex::Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(Ei, TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        auto const & rhon   = rho_n.const_array(mfi);
        const int stage = rho_stage_comp;
        auto const dlo = amrex::lbound(dom_nd);
        auto const dhi = amrex::ubound(dom_nd);
        amrex::ParallelFor(mfi.tilebox(),
        [=] AMREX_GPU_DEVICE (int i, int j, int k)
        {
            amrex::Real const rho_stage = rhoa(i, j, k, stage);
            // True vacuum carries no electron fluid: zero the term there
            // (density-floored cells KEEP their inertia; the fluid recast
            // never reaches this branch -- its density is floored
            // positive).
            if (rho_stage <= 0.0_rt) {
                ei(i,j,k,0) = 0.0_rt;
                ei(i,j,k,1) = 0.0_rt;
            ...
            amrex::Real const rho_lim = amrex::max(rho_stage, rho_floor);
                              ...
                              int c) -> amrex::Real
            {
                amrex::Real const rl = amrex::max(
                    amrex::max(rhoa(ii,jj,kk,stage), 0.0_rt), rho_floor);
...
void HybridPICModel::RotateElectronInertiaHistory (amrex::Real a_theta)
{
    ...
    amrex::MultiFab::LinComb(Je_n,
        ...
        0, 3, Je_n.nGrowVect());
    // Re-arm the per-step frozen rho^n capture (hybrid deposit mode; the
    // recast solver refills its exact image every step regardless).
