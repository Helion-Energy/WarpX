                                                     ...
                                                     const bool skip_in_initial_field)
{
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        mode == RefreshMode::All || !skip_in_initial_field,
        "UpdateHybridExternalFields(CircuitOnly) cannot skip in_initial_field coils");

    // The parser-driven partial sums are cached (and reused by CircuitOnly
    // refreshes) only for the full-coil-set refresh: the in_initial_field
    // subset refresh of AddInitialExternalBField accumulates directly and
    // leaves the cache alone (the full refresh that follows it rebuilds it).
    const bool use_static_cache = m_any_python_scale && !skip_in_initial_field;
                                ...
                                ablastr::fields::MultiLevelVectorField& dst_B)
    {
        ...
        for (int lev = 0; lev <= warpx.finestLevel(); ++lev) {
            AddExternalFieldFromVectorPotential(dst_E[lev], scale_factor_E, A_ext[lev], warpx.GetEBUpdateEFlag()[lev]);
            AddExternalFieldFromVectorPotential(dst_B[lev], scale_factor_B, curlA_ext[lev], warpx.GetEBUpdateBFlag()[lev]);
        }
    };
    ...
    if (mode == RefreshMode::All) {
        if (use_static_cache) {
            ...
            // Zero E and B external fields prior to accumulating external
            // fields, then iterate over the external fields and add them
            // together with their individual time functions.
            for (int lev = 0; lev <= warpx.finestLevel(); ++lev) {
                for (int idir = 0; idir < 3; ++idir) {
