...
Array<LinOpBCType,AMREX_SPACEDIM> ImplicitSolver::convertFieldBCToLinOpBC (const Array<FieldBoundaryType,AMREX_SPACEDIM>& a_fbc, const int bdry_side) const
{
    ...
    for (int i = 0; i < AMREX_SPACEDIM; i++) {
        if (a_fbc[i] == FieldBoundaryType::PML) {
            ...
            // "None" is the r = 0 axis of RZ simulations (and a boundary whose
            // values are left untouched elsewhere). The linear operator sees a
            // symmetry (Neumann) closure there, the same approximation used for
            // the Silver-Mueller and current-driven insulator boundaries.
            ablastr::warn_manager::WMRecordWarning("Implicit solver",
                "With a 'None' field boundary (the RZ axis), the Curl-Curl preconditioner uses a "
                "symmetry boundary as an approximation; the nonlinear residual is unaffected.",
                ablastr::warn_manager::WarnPriority::medium);
            lbc[i] = LinOpBCType::symmetry;
        } else if (a_fbc[i] == FieldBoundaryType::Open) {
            // The open (free-space) boundary has no exact linear-operator
            // counterpart. Inside the preconditioner only the quality of the
            // Newton direction depends on it, so a Dirichlet closure on the
            // boundary row is used; the residual keeps the true open boundary.
            ablastr::warn_manager::WMRecordWarning("Implicit solver",
                "With an Open field boundary, the Curl-Curl preconditioner uses a Dirichlet "
                "boundary as an approximation; the nonlinear residual is unaffected.",
                ablastr::warn_manager::WarnPriority::medium);
            lbc[i] = LinOpBCType::Dirichlet;
        } else {
...
void ImplicitSolver::PreLinearSolve ()
{
    ...
    if (m_use_mass_matrices) {
        ...
        // whenever that did not happen (fused path off, or the evaluation was a
        // line-search trial whose residual the solver reused).
        if (!(m_fused_mass_matrices_deposit && m_mass_matrices_current)) {
            m_WarpX->DepositMassMatrices();
            m_mass_matrices_current = true;
        }
                                ...
                                const bool         a_from_jacobian )
{
    ...
    // Fused mass-matrix deposit: on the nonlinear residual evaluations that
    // deposit J (never in the linear stage), zero the mass matrices here and let
    // the push-and-deposit pass fill them alongside J.
    options.deposit_mass_matrices = m_use_mass_matrices && m_fused_mass_matrices_deposit
                                    && !a_from_jacobian && m_mass_matrices_wanted;
