...
void ThetaImplicitHybrid::Define (WarpX* const a_WarpX, bool a_from_restart)
{
    ...
    // Initialize the solver state. E_old is checkpointed for implicit restarts.
    m_E.Copy(FieldType::Efield_fp);
    m_Eold.Copy(
        a_from_restart ? FieldType::E_old : FieldType::Efield_fp,
        FieldType::None,
        true);
                                  ...
                                  const int a_step)
{
    ...
    // Save E^n for restart and for the time-centered residual.
    SaveEoldMultifab();
