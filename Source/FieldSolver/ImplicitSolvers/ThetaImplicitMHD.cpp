...
void ThetaImplicitMHD::Define (WarpX* const warpx, const bool from_restart)
{
    ...
    // is bit-identical to no wall.
    {
        ...
        // The no-slip wall is a VISCOUS condition -- the tangential wall
        // shear mu u_t/(dn/2) carried by the contour faces. Without a
        // viscous coefficient there is no shear to carry and the knob
        // would be a silent no-op (the free-slip wall it is meant to
        // replace).
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            !m_wall_mask.NoSlip() ||
                m_viscosity > 0.0_rt || m_wall_viscosity_band_value > 0.0_rt,
            "implicit_mhd.wall_no_slip is a viscous FACE condition (the "
            "tangential wall shear at the shaped contour) and requires a "
            "viscous coefficient: set implicit_mhd.viscosity, or "
            "implicit_mhd.wall_viscosity_band_value with "
            "wall_viscosity_mask covering the contour (the reference code's small_vis "
            "pedestal). With neither, the wall stays free-slip");
...
void ThetaImplicitMHD::BookPinnedDefect (const WarpXSolverVec& a_residual)
{
    ...
    // U^n)/theta turns it into a creation of F_i/theta per unit volume.
    // Book it with the cell measure -- the supply ledger's conventions:
    // conserved blocks only (the dual-energy U_i block is bookkeeping),
    // the frozen ion fluid gets no mass booking, wall-masked cells are
    // excluded, RZ annulus weight in-kernel; units kg and J (per unit
    // cross-section in 1D). Signed: a pinned component whose residual
    // turned inward by the exit was held BELOW where the equations
    // wanted it and books a removal.
    const bool dual_energy_closure = m_ion_closure == "dual_energy";
    const bool total_energy_closure =
        m_ion_closure == "total_energy" || dual_energy_closure;
    const bool cgl_closure = m_ion_closure == "cgl";
    ...
    const int num_booked_blocks = dual_energy_closure ? 3 : num_blocks;
    ...
    amrex::Real step_totals[2] = {0.0_rt, 0.0_rt}; // mass, energy
    for (int block = first_block; block < num_booked_blocks; ++block) {
        ...
        const amrex::Real block_total =
            inverse_theta * cell_volume *
            amrex::get<0>(reduce_data.value(reduce_op));
        step_totals[block == 0 ? 0 : 1] += block_total;
    }
    amrex::ParallelAllReduce::Sum(step_totals, 2,
                                  amrex::ParallelContext::CommunicatorSub());
    m_pinned_defect_mass += step_totals[0];
    m_pinned_defect_energy += step_totals[1];
    amrex::Print() << "MHD pinned-defect ledger: booked mass "
                   << step_totals[0] << " energy " << step_totals[1]
                   << " this solve over " << m_projected_components
                   << " pinned components (cumulative "
                   << m_pinned_defect_mass << ", " << m_pinned_defect_energy
                   << ")\n";
}
...
void ThetaImplicitMHD::WriteFloorLedgerRow (const int step)
{
    ...
    if (m_active_set_mode) {
        ...
        // (BookPinnedDefect) as two extra columns.
                                                      ...
                                                      const amrex::Real time) const
{
    ...
    // antisymmetric tangential viscous image), not a volumetric
    // constraint, so there is nothing for the cell-centered increment
    // kernel below to mask. The wall-adjacent LIVE cells keep their
    // three momentum rows as ordinary unknowns -- free to be advected,
    // to be replenished by their neighbours, and to lift off the wall.
    ...
    for (amrex::MFIter mfi(density); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(box, [=] AMREX_GPU_DEVICE (int i, int j, int k) {
            ...
            // The momentum rows share the rigid-conductor envelope of
            // the other fluid channels and nothing else: the no-slip
            // wall acts through the face stress registers, not here.
            const amrex::Real momentum_live = wall_live;
