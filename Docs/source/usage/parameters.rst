...
.. pp:param:: hybrid_pic_model.use_conformal_eb
    ...
    When combined with :pp:param:`hybrid_pic_model.use_global_A_recovery`, note that the vacuum blending
    overwrites the conformally-evolved B (and reconstructed E) on cut faces in vacuum-classified cells
    with values derived from the recovered A, which are only stair-step accurate at the wall.
...
.. pp:param:: hybrid_pic_model.use_global_A_recovery
    ...
    If :pp:param:`algo.maxwell_solver` is set to ``hybrid``, this enables the global vector potential recovery.
    After each half-step of the B-field advance, the vector potential :math:`\vec{A}` generated by the plasma
    currents is recovered by solving the vector Poisson equation :math:`\vec{\nabla}^2 \vec{A} = -\mu_0 \vec{J}`,
    where :math:`\vec{J} = \vec{\nabla} \times \vec{B} / \mu_0 - \vec{J}_{ext}` is masked to zero in vacuum
    cells (:math:`\rho \leq 0`). Since the hybrid solver evolves only the plasma-generated part of the fields
    (any external field is split off), homogeneous Dirichlet boundary conditions are applied on all
    non-periodic domain boundaries and on embedded boundaries. The B field is then replaced by
    :math:`\vec{\nabla} \times \vec{A}` in vacuum cells (:math:`\rho < n_{floor} q_e`) where the local
    Faraday update is unreliable, and after the final Ohm's law solve the E field in vacuum cells is
    reconstructed from :math:`\vec{E} = -\partial_t \vec{A} - \vec{\nabla} P_e / \rho` using BDF2 time
    differencing of the recovered A history. Requires a meaningful :pp:param:`hybrid_pic_model.n_floor`
    and adds two vector Poisson solves per timestep. Supported in 3D and RZ (m=0) geometries.

.. pp:param:: hybrid_pic_model.global_A_rtol
    :type: ``float``
    :default: ``1e-8``
    :optional:

    If :pp:param:`hybrid_pic_model.use_global_A_recovery` is ``true``, this sets the relative tolerance
    of the vector Poisson solve.

.. pp:param:: hybrid_pic_model.global_A_atol
    :type: ``float``
    :default: ``0``
    :optional:

    If :pp:param:`hybrid_pic_model.use_global_A_recovery` is ``true``, this sets the absolute tolerance
    of the vector Poisson solve.

.. pp:param:: hybrid_pic_model.global_A_max_iters
    :type: ``int``
    :default: ``100``
    :optional:

    If :pp:param:`hybrid_pic_model.use_global_A_recovery` is ``true``, this sets the maximum number of
    MLMG iterations of the vector Poisson solve.

.. pp:param:: hybrid_pic_model.global_A_verbosity
    :type: ``int``
    :default: ``0``
    :optional:

    If :pp:param:`hybrid_pic_model.use_global_A_recovery` is ``true``, this sets the verbosity of the
    MLMG solver used for the vector Poisson solve.
