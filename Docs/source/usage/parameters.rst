...
.. pp:param:: hybrid_pic_model.isotropic_operators
    ...
    * the **electron-pressure gradient** uses the transverse-smoothed staggered difference
    ...
    term (fully isotropic on cubic cells, consistent on non-cubic cells). Within a corner
    reach (:math:`(\sqrt{d}+1/2)\,h`) of an embedded boundary each operator automatically
    falls back per point to its standard compact stencil, whose reads the EB masks and
    fills maintain. Cartesian geometries only; in RZ the standard cylindrical operators are
    used (the axisymmetric resistive operator has no such anisotropy).
