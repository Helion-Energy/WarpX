    ...
    * ``theta_implicit_em``: Use a :math:`\theta`-implicit electromagnetic solver.
      ...
      - **Nonlinear solvers:**
        ...
        - ``implicit_evolve.fused_vector_ops`` (``int``, default: 0)
          Kernel fusion level of the solver-vector operations behind the nonlinear and linear solvers. ``0``: one kernel per MultiFab of the state per vector operation (and one host synchronization per MultiFab for every inner product). ``1``: the elementwise operations (copy, increment, linear combination, scale, set, pack/unpack of the serialized vector) run as ONE kernel over a fused layout of every box of every MultiFab of the state; the per-element expressions are those of the AMReX MultiFab routines, so the results are bit-identical on CPU builds (verified over the whole implicit MHD test suite); on GPU builds the identity is not testable (the default path is itself not run-to-run reproducible there) and the compiler may contract fused multiply-adds differently in the two kernels, so treat level 1 as "identical up to round-off" on GPUs. ``2``: the inner product also runs as one kernel plus one reduction (the summation order changes, at round-off level). Levels 1 and 2 remove the per-MultiFab launch latency and synchronizations that dominate the Krylov bookkeeping of a multi-block state on a GPU (the theta-implicit MHD state has about a dozen MultiFabs).

        - ``implicit_evolve.fused_residual`` (``int``, default: 0)
          One launch per stage for the bookkeeping of the theta-implicit MHD residual and of the ``pc_mhd_block`` preconditioner. The residual-side fusions (the state-to-field copies, ghost fills, interpolations, zero fills, residual assembly and active-set mask operations of ``ThetaImplicitMHD``) are wired into the RZ conservative-form path (``implicit_mhd.fluid_flux = hlld`` or ``central``) only and are off elsewhere (a banner says so); the preconditioner's fused block copies and frozen-row restores, the direct solvers' fused component gathers/scatters and the packed-GMRES unpack's skipped no-op exchanges follow the knob in every configuration. All of it is exact. ``0``: one kernel per MultiFab for every block copy, ghost-row clamp/mirror, staggered interpolation and zero fill, and a ``FillBoundary``/``FillBoundaryAndSync`` call per MultiFab per stage. ``1``: the copies of the state into the field registers, the axial and radial domain-ghost fills, the cell-centered interpolations of the currents and fields, the register zero fills, the residual assembly ``B - B^n``, the active-set mask operations and the preconditioner's block copies and frozen-row restores each run as ONE kernel over all the MultiFabs of the stage, and the ghost exchanges that cannot move data (one box in a non-periodic domain, the single-GPU production layout) are skipped. The per-element expressions are unchanged (pure copies, sign flips, the same interpolation calls), so the results are bit-identical on CPU builds (verified over the whole implicit MHD suite: every test's Newton diagnostic rows identical to the last digit) and identical up to the compiler's fused-multiply-add choices on GPUs, exactly like ``fused_vector_ops = 1``; the direct solvers' fused gathers/scatters exist on the cuDSS (GPU) path only and are verified by the production twins' solve statistics. On a 1024 x 128 RZ mesh one residual evaluation drops from about 170 kernel launches plus about 55 no-op ghost exchanges to about 60 launches. ``2``: level 1 plus two residual-only savings that are exact by construction: (a) the boundary application inside the residual's Faraday update is skipped -- the fused residual reads ``B - B^n`` on the valid faces and re-sets B and re-applies the full boundary before anything reads a ghost cell again; it is skipped whole on a deck whose fills all write ghost cells only (PMC, on-axis and r_hi Green's fills: no open z cap, no PEC, no insulator boundary), and reduced to the skip of the r_hi-only Green's-function refill otherwise (an open z cap keeps its own fill: the next application's source deposit reads the cap ghost row it wrote, a lagged recursion that is measurably not skippable; a PEC fill writes the valid normal face value, which the Faraday update reproduces only where the tangential E is exactly zero -- measurably not on decks with a Hall term or a ramped drive); and (b) where the state holds one box in a non-periodic domain (the one-GPU production layout: every kernel of a stage runs on one stream in issue order and no ghost exchange can move data) and the solver runs the RZ conservative-form path, the per-MFIter stream synchronizations are elided inside the residual evaluation, the active-set mask operations and the block preconditioner's apply (the preconditioner arms it from the solver's decision; its MLMG applications and the circuit hook keep their synchronizations); the banner prints the arming decision. Bit-identical on CPU builds (the elision is a no-op there; verified over the whole implicit MHD suite and on 4-rank x 8-box layouts); on GPUs the elision removes the idle gap the host-side synchronization opened after every small kernel. A former level 3 (the face-flux evaluation split into an advective and a diffusive launch) was measured slower than level 2 -- the diffusive closure alone holds the register budget -- and removed; the value 3 is refused.
        ...
        - ``implicit_evolve.nonlinear_solver = newton``: Use a PS-JFNK method. Required for large time steps, but efficiency often relies on preconditioning and/or using ``implicit_evolve.use_mass_matrices_jacobian = true``.
          ...
          - ``newton.linear_solver`` (``string``, default: "amrex_gmres") Other accepted values: "packed_gmres", "petsc_ksp".
            ...
            - ``packed_gmres.gemv`` (``string``, default: ``auto``): ``blas`` (cuBLAS; CUDA builds configured with ``WarpX_CUBLAS=ON``, the default there), ``native`` (portable fused kernels: a deterministic two-stage reduction for the inner products, one fused pass for the updates; on the host a fixed 64-chunk partial sum independent of the OpenMP thread count), or ``auto`` (``blas`` when available; HIP builds and CUDA builds without cuBLAS fall back to ``native`` silently -- the banner ``PackedGMRES: ... through native kernels`` is the indication; SYCL builds abort at the solver's definition, the native device kernels are CUDA/HIP only).
...
.. pp:param:: implicit_mhd.ion_closure
    ...
    channel books (the viscous heating as the kinetic energy the face
    stress removes, see :pp:param:`implicit_mhd.dual_energy_viscous_heating`;
    the ion conductive flux verbatim, the pedestal-band internal drain,
    the floor machinery); KE-specific terms (Lorentz
...
.. pp:param:: implicit_mhd.dual_energy_sync
    :type: ``bool``
    :default: ``1`` (on)

    With ``ion_closure = dual_energy``: switch for the end-of-step
    re-sync as a whole. ``0`` disables BOTH legs (the mixmaster
    :math:`E_i` rewrite and the Enzo-style :math:`U_i := E_i - K`
    overwrite), leaving :math:`E_i` and :math:`U_i` as independent
    registers. A VERIFICATION switch: with the sync on, a thermal
    cell's :math:`U_i` is overwritten from :math:`E_i` at every step
    end, so no diagnostic can see what :math:`U_i`'s own sources
...
.. pp:param:: implicit_mhd.dual_energy_viscous_heating
    ...
    With ``ion_closure = dual_energy``: how the internal register
    :math:`U_i` receives the viscous heating.

    ``stress_work`` books, for every face, the dissipation
    :math:`Q_f = -\Pi_f\cdot(\boldsymbol{u}_R-\boldsymbol{u}_L)/\Delta n`
    formed from the very same face stress :math:`\Pi_f` the momentum
    equation carries -- same coefficient (including the wall viscosity
    band, :pp:param:`implicit_mhd.wall_viscosity_band_value`), the same
    no-slip antisymmetric image, the same open-region multiplier, the
    same free-streaming cap (:pp:param:`implicit_mhd.viscous_flux_limit_factor`)
    and the same :pp:param:`implicit_mhd.viscous_theta` stage -- paired
    with the theta-stage cell velocity difference the discrete
    ...
    viscosity. It ignores the wall viscosity band (with
    ``wall_viscosity_band_value = 0`` it heats faces that carry NO
    stress; with a positive band value it heats at :math:`\rho\nu`
    where the stress acts at the band's :math:`\mu`), the no-slip image
    (a quarter of the removed kinetic energy at an interface face), the
    free-streaming cap, and the ``viscous_theta`` stage (at
    ``viscous_theta = 1`` it heats by :math:`1 + \nu k^2\Delta t/2`
    times the kinetic energy removed). Kept only to reproduce runs that
    flew it.
...
.. pp:param:: implicit_mhd.energy_audit_file
    ...
    :math:`\theta = 1`), the Faraday/boundary defect, the Newton defect per
    ...
    the stages of the state update, the between-step injections, and the
    code's own cumulative ledgers. Two closure residuals are written:
    ``resid_booked`` (the physical fluxes and the code's existing bookings
    only -- what the ledgers leave unexplained) and ``resid_full`` (every
    named term; round-off by construction when nothing is missing), per
...
.. pp:param:: implicit_mhd.energy_audit_interval
    ...
    single-step budget of that step (the cumulative columns then sum the
    audited steps only) and the between-step ``inject_*`` columns are
    defined only when consecutive steps are audited.
