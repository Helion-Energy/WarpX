...
class ThetaImplicitMHDEvolveScheme(picmistandard.base._ClassWithInit):
    ...
    wall_heat_flux_cap: {"none", "free_streaming", "sonic"}, optional
        Cap on the WALL conductive exchanges -- the shaped-wall interface
        drain of every reservoir wall_thermal_bc mode (outflow_limited,
        dirichlet, dirichlet_limited) and the z-end exchange
        (z_wall_conduction), in both species channels. An OVERRIDE of
        the per-mode defaults; unset (the default) keeps every existing
        mode bit-identical: "dirichlet" uncapped, "dirichlet_limited" /
    ...
    dual_energy_sync: bool, default=True
        With ion_closure="dual_energy": switch for the end-of-step re-sync
        as a whole. False disables both legs (the mixmaster E_i rewrite
        and the Enzo U_i := E_i - KE overwrite), leaving E_i and U_i as
    ...
    dual_energy_viscous_heating: str, default="stress_work"
        ...
        receives the viscous heating. "stress_work" books per face the
        dissipation -Pi_f . (u_R - u_L)/dn formed from the very same face
        stress the momentum equation carries (same coefficient including
        ...
        the stage (1 + nu k^2 dt/2 too much heat at viscous_theta = 1);
        kept only to reproduce runs that flew it.
    ...
    wall_viscosity_mask: bool, default=False
        ...
        wall_model): substitutes one viscous face coefficient -- for the
        momentum stress AND its heating work, which share one
        coefficient assembly -- at faces within wall_viscosity_mask_width
        cells of the masked shaped-wall contour, so no viscous
    ...
    wall_viscosity_mask_width: int, default=2
        ...
        the masked contour (Chebyshev distance over the stair-step
        tables).

    wall_viscosity_band_value: float, default=0 (the legacy zero band)
        Viscous face coefficient substituted on the band faces, as an
        ABSOLUTE DYNAMIC viscosity [Pa s] -- deliberately not scaled by
        density, unlike the interior's rho_f * viscosity (the
        `viscosity` knob is kinematic-style, m^2/s). 0 skips the viscous
        block on band faces entirely (bit-identical to the pre-band
        code); the reference code's parity value is 1e-4 Pa s (small_vis,
        step.f90:90), a pedestal that caps the coefficient where
    ...
    fused_residual: int, default=0
        ...
        2 = level 1 plus the exact residual-only skips (the dead
        open-boundary ghost refill of the Faraday update; the per-MFIter
        stream synchronizations on a single local box), 3 = level 2 plus
        the face-flux evaluation split into an advective and a diffusive
        launch per direction (same expressions, bit-identical on CPU).
    """
    ...
    def solver_scheme_initialize_inputs(self):
        ...
        implicit_mhd.vacuum_reference_base_density = self.vacuum_reference_base_density
