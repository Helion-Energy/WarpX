...
class HybridPICSolver(picmistandard.base._ClassWithInit):
    ...
    use_conformal_eb: bool, default=False
        If True, use the conformal embedded-boundary Faraday update for the
        B-field push (enlarged cell technique of `Xiao and Liu (2004)
        <https://doi.org/10.1109/LMWC.2004.837385>`_, shared with the ``ect``
        Maxwell solver) instead of the stair-step approximation. Faces cut by
        the embedded boundary are advanced using the conformal circulation of
        E over the uncovered face area and small faces are enlarged into their
    ...
    use_global_A_recovery: bool, default=False
        If True, after each half-step of the B-field advance the global
        vector potential A generated by the plasma currents is recovered by
        solving the vector Poisson equation ``del^2 A = -mu0*J`` (with the
        current masked to zero in vacuum cells and homogeneous Dirichlet
        boundary conditions on all non-periodic domain boundaries and
        embedded boundaries). The B field is then replaced by ``curl(A)``
        in vacuum cells (``rho < n_floor*q_e``) where the local Faraday
        update is unreliable, and the E field in vacuum cells is
        reconstructed from ``E = -dA/dt - grad(Pe)/rho`` using BDF2 time
        differencing of the recovered A. Requires a meaningful ``n_floor``
        and adds two vector Poisson solves per timestep. Supported in 3D
        and RZ (m=0) geometries.

    global_A_rtol: float, default=1e-8
        Relative tolerance of the global A recovery vector Poisson solve.
        Only used when ``use_global_A_recovery=True``.

    global_A_atol: float, default=0.
        Absolute tolerance of the global A recovery vector Poisson solve.
        Only used when ``use_global_A_recovery=True``.

    global_A_max_iters: int, default=100
        Maximum number of MLMG iterations for the global A recovery solve.
        Only used when ``use_global_A_recovery=True``.

    global_A_verbosity: int, default=0
        Verbosity of the MLMG solver used for the global A recovery solve.
        Only used when ``use_global_A_recovery=True``.
    ...
    **Constraints and Limitations:**
    ...
    def __init__(
        ...
        use_global_A_recovery=None,
        global_A_rtol=None,
        global_A_atol=None,
        global_A_max_iters=None,
        global_A_verbosity=None,
        ...
        **kw,
        ...
        self.use_global_A_recovery = use_global_A_recovery
        self.global_A_rtol = global_A_rtol
        self.global_A_atol = global_A_atol
        self.global_A_max_iters = global_A_max_iters
        self.global_A_verbosity = global_A_verbosity
    ...
    def solver_initialize_inputs(self):
        ...
        pywarpx.hybridpicmodel.use_global_A_recovery = self.use_global_A_recovery
        pywarpx.hybridpicmodel.global_A_rtol = self.global_A_rtol
        pywarpx.hybridpicmodel.global_A_atol = self.global_A_atol
        pywarpx.hybridpicmodel.global_A_max_iters = self.global_A_max_iters
        pywarpx.hybridpicmodel.global_A_verbosity = self.global_A_verbosity
        pywarpx.hybridpicmodel.__setattr__(
